...
exports.migrateOldMessagesToNewSchema = migrateOldMessagesToNewSchema;
...
async function migrateOldMessagesToNewSchema() {
    try {
        console.log('🚀 Starting migration from Message to Usage/UsageLog...');
        // Get all old messages
        const oldMessages = await Message_1.default.find({}).sort({ timestamp: 1 }).lean();
        ...
        const trackers = await Tracker_1.default.find({}).lean();
        const trackerMap = new Map();
        for (const tracker of trackers) {
            trackerMap.set(tracker._id.toString(), (0, usageLogger_1.createTrackerSnapshot)(tracker));
        }
        ...
        for (let i = 0; i < oldMessages.length; i += batchSize) {
            ...
            for (const message of batch) {
                ...
                usageLogPromises.push(UsageLog_1.default.create({
            ...
            for (const usage of usageUpdateMap.values()) {
                usagePromises.push(Usage_1.default.findOneAndUpdate({
                    userId: usage.userId,
                    date: usage.date,
