...
exports.getTrackerLogs = exports.getTrackerUsage = exports.getOverallUsage = void 0;
 ...
 */
const getOverallUsage = async (req, res) => {
    try {
        ...
        const overallStats = await Usage_1.default.aggregate([
            {
        ...
        const byTracker = await Usage_1.default.aggregate([
            {
                $match: { userId: userObjectId }
            },
            {
        ...
        const recentActivity = await Usage_1.default.aggregate([
            {
                $match: {
                    userId: userObjectId,
...
exports.getOverallUsage = getOverallUsage;
/**
 * Get usage statistics for a specific tracker
 */
const getTrackerUsage = async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        const trackerStats = await Usage_1.default.aggregate([
            {
                $match: {
                    userId: userObjectId,
        ...
        if (trackerStats.length === 0) {
            ...
            const TrackerModel = (await Promise.resolve().then(() => __importStar(require('../models/Tracker')))).default;
        ...
        // Get daily usage
        const dailyUsage = await Usage_1.default.aggregate([
            {
                $match: {
                    userId: userObjectId,
                    'trackerSnapshot.trackerId': trackerId
        ...
        const messages = await UsageLog_1.default.find({
            userId: userObjectId,
            'trackerSnapshot.trackerId': trackerId
        })
            .sort({ timestamp: -1 })
...
exports.getTrackerUsage = getTrackerUsage;
/**
 * Get logs for a specific tracker
 */
const getTrackerLogs = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { trackerId } = req.params;
        ...
        // Get total count
        const totalCount = await UsageLog_1.default.countDocuments({
            userId: userObjectId,
            'trackerSnapshot.trackerId': trackerId
        });
        // Get paginated logs
        const logs = await UsageLog_1.default.find({
            userId: userObjectId,
            'trackerSnapshot.trackerId': trackerId
        })
            .sort({ timestamp: -1 })
